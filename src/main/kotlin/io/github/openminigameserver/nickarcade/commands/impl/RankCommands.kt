    ...
    fun listPlayersRanked(
        ...
        val players =
            PlayerDataManager.playerDataCollection.find(
                and(
                    (ArcadePlayerData::overrides / PlayerOverrides::rankOverride).exists(),
                    ArcadePlayerData::overrides / PlayerOverrides::rankOverride eq rank
                )
            ).toList().map { ArcadePlayer(it) }
    ...
    private fun <T> handleValueSet(
    ...
    ): (commandContext: @NonNull CommandContext<ArcadeSender>) -> Unit =
        {
            command(it.sender, HypixelPackageRank.ADMIN) {
                ...
                prop.set(player.data.overrides, valueFinal)
